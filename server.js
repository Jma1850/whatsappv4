...
const pickLang=txt=>{
  const m=txt.trim(); const d=m.match(/^\d/);
...
const toWav=(i,o)=>new Promise((res,rej)=>
  ffmpeg(i).audioCodec("pcm_s16le").outputOptions(["-ac","1","-ar","16000","-f","wav"])
...
const detectLang=async q=>(await fetch(
